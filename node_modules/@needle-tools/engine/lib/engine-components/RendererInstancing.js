    ...
    */
    ...
    add(handle) {
        ...
        if (!this.validateGeometry(geo)) {
            if (debugInstancing)
                console.error("Cannot add instance, invalid geometry", this.name, geo);
            return;
        }
     ...
     */
    mustGrow(geo) {
        if (this.count >= this.maxInstanceCount)
            return true;
        if (!geo)
            return false;
    ...
    addGeometry(handle) {
        ...
        if (smallestBucket != null) {
            ...
            this.inst.setGeometryAt(bucket.index, handle.object.geometry);
