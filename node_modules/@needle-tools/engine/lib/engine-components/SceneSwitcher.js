    ...
    */
    ...
    async __internalSwitchScene(scene) {
        if (this._currentScene) {
            if (debug)
                console.log("UNLOAD", this._currentScene.url, "HasURL?: " + this._currentScene.hasUrl);
            const sceneListener = this.tryGetSceneEventListener(this._currentScene.asset);
    ...
    async onStartLoading() {
        ...
        if (this.loadingScene) {
            ...
            if (this._isCurrentlyLoading && this.loadingScene?.asset) {
                if (debug)
                    console.log("Add loading scene", this.loadingScene.uri, this.loadingScene.asset);
    ...
    async onEndLoading() {
        ...
        if (this.loadingScene?.asset) {
            if (debug)
                console.log("Remove loading scene", this.loadingScene.uri);
    ...
    tryGetSceneEventListener(obj, level = 0) {
        const sceneListener = GameObject.foreachComponent(obj, c => c.sceneClosing ? c : undefined);
