...
export class PlayAnimationOnClick extends Behaviour {
    ...
    afterCreateDocument(ext, context) {
        ...
        document.traverse(model => {
            if (model.uuid === this.target?.uuid) {
                const sequence = PlayAnimationOnClick.getActionForSequences(model, this.animationSequence, this.animationLoopAfterSequence);
    ...
    static getActionForSequences(model, animationSequence, animationLoopAfterSequence) {
        const getOrCacheAction = (model, anim) => {
            ...
            if (!action) {
                action = ActionBuilder.startAnimationAction(model, anim.start, anim.duration, anim.speed);
