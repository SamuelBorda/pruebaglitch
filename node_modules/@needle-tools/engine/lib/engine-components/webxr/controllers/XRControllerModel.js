...
export class XRControllerModel extends Behaviour {
    ...
    async loadHandModel(controller) {
        ...
        if (customHand) {
            if (!customHand.uri.includes(expectedHandModelName)) {
                ...
                if (debug)
                    console.log("XRControllerModel: loading custom hand model from " + basePath);
            }
        }
        const handObject = new Object3D();
        setDontDestroy(handObject);
        // @ts-ignore
        const handmesh = new XRHandMeshModel(handObject, hand, loader.path, controller.inputSource.handedness, loader, (object) => {
