...
import { AxesHelper, Matrix4, Object3D, Quaternion, Ray, Vector3 } from "three";
...
export class NeedleXRController {
    ...
    */
    ...
    model = null;
    _debugAxesHelper = new AxesHelper(.2);
...
C:${this.connected ? "yes" : "no"} T:${this.isTracking ? "yes" : "no"} Hand:${this.inputSource.hand ? "yes" : "no"}`;
    ...
    onUpdateFrame(frame) {
        ...
        if (hand) {
            ...
            const writePose = wrist && this.getHandJointPose(wrist, frame);
            if (writePose) {
                gotWrist = true;
